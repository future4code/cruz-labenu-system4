...
import {Class, ClassName} from '../types';
...
export default async function createClass(req:Request,res:Response){
    ...
    try{
        ...
        const Module : number = reqBody.Module;
        ...
        if (!Object.values(ClassName).includes(Class_Name)) {
            throw new Error ('Please, check your information! You need complete with one of then: EPPS, CRUZ, DUMMONT, MOLINA, JACKSON')
        }
